...
public class GiftcardResource {
    ...
    @Autowired
    public GiftcardResource(ApplicationProperty applicationProperty) {
        ...
        client.setApplicationName("adyen-java-spring-online-payments giftcard-example adyen-web/5.33.0");
    ...
    public ResponseEntity<CreateCheckoutSessionResponse> sessions(@RequestHeader String host, @RequestParam String type, HttpServletRequest request) throws IOException, ApiException {
        ...
        var amount = new Amount()
            .currency("EUR")
            .value(11000L); // value is 110€ in minor units
        ...
        checkoutSession.setLineItems(Arrays.asList(
            new LineItem().quantity(1L).amountIncludingTax(5500L).description("Sunglasses"),
            new LineItem().quantity(1L).amountIncludingTax(5500L).description("Headphones"))
        );
